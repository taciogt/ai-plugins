...
Before launching `task-planner`, invoke the `deliver:task-refinement` skill with `$ARGUMENTS`.
...
Review the plan for Signal 5: does it introduce a technology, library, or architectural pattern not currently in the codebase? If yes, invoke `deliver:task-refinement` with `$ARGUMENTS signal5:<finding>` before presenting the plan to the user.
